...
describe('Header Decorator tests', () => {
  describe('RequiredHeaders decorator', () => {
    ...
    it('should convert headers to lowercase', () => {
      ...
      expect(metadata?.headers).toEqual(['authorization', 'content-type', 'x-api-key']);
    });
  ...
  describe('SetHeaders decorator', () => {
    ...
    it('should attach set headers metadata with dynamic values', () => {
      const dynamicValue = (req: Request, _res: Response) => `User-${req.params.id}`;
  ...
  describe('Metadata getters', () => {
    it('should return undefined for methods without decorators', () => {
      ...
      expect(getRequiredHeadersMetadata(instance, 'testMethod')).toBeUndefined();
    ...
    it('should return undefined for non-existent methods', () => {
      ...
      expect(getRequiredHeadersMetadata(instance, 'nonExistent')).toBeUndefined();
    });
  });
  ...
  describe('createRequiredHeadersMiddleware', () => {
    ...
    it('should call next when all required headers are present', () => {
      ...
      const middleware = createRequiredHeadersMiddleware(['authorization', 'content-type']);
    ...
    it('should return 400 with all missing headers', () => {
      ...
      const middleware = createRequiredHeadersMiddleware(['authorization', 'content-type', 'x-api-key']);
      ...
      expect(mockRes.json).toHaveBeenCalledWith({
        success: false,
        error: 'Missing required headers: authorization, content-type, x-api-key',
      });
    });
  ...
  describe('createSetHeadersMiddleware', () => {
    ...
    it('should set static headers', () => {
      ...
      expect(mockRes.setHeader).toHaveBeenCalledWith('X-Custom', 'custom-value');
      expect(mockNext).toHaveBeenCalledOnce();
    });
    ...
    it('should call dynamic functions with req and res', () => {
      ...
      expect(mockRes.setHeader).toHaveBeenCalledWith('X-Dynamic', 'dynamic-value');
    });
  });
  ...
  describe('Combined decorators', () => {
    it('should allow multiple header decorators on same method', () => {
      ...
      const instance = new TestController();
      const requiredHeaders = getRequiredHeadersMetadata(instance, 'testMethod');
    ...
    it('should allow cookie and header decorators together', () => {
      ...
      expect(getRequiredCookieMetadata(instance, 'testMethod')?.name).toBe('sessionId');
      expect(getRequiredHeadersMetadata(instance, 'testMethod')?.headers).toEqual(['authorization']);
      expect(getSetHeadersMetadata(instance, 'testMethod')?.headers[0]?.name).toBe('X-Custom');
    });
    ...
    it('should allow SetCookie with other decorators', () => {
      ...
      expect(getSetCookieMetadata(instance, 'testMethod')?.name).toBe('newSession');
      expect(getSetCookieMetadata(instance, 'testMethod')?.options?.httpOnly).toBe(true);
      expect(getSetHeadersMetadata(instance, 'testMethod')?.headers[0]?.value).toBe('true');
    });
  });
