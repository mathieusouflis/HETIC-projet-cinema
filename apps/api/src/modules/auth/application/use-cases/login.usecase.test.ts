...
describe("LoginUseCase", () => {
  ...
  const useCase = new LoginUseCase(mockedUserRepository, passwordService, tokenService)

  it("should throw an error when user not found", async () => {
    const email = "fakeEmail@gmail.com";
    await expect(useCase.execute({ email, password: "fakePassword" })).rejects.toThrow();
  });

  it("should throw an error when wrong password", async () => {
    const email = "test1@example.com";
    const password = "fakePassword123";
    await expect(useCase.execute({ email, password: password })).rejects.toThrow();
  });
