...
import { UpdateWatchpartyBody } from "../dto/request/update-watchparty.body.validator.js";
...
export class UpdateWatchpartyUseCase {
  ...
  async execute(userId: string, id: string, body: UpdateWatchpartyBody): Promise<Watchparty> {
    ...
    if (!watchparty.isCreator(userId) && !watchparty.isLeader(userId)) {
      throw new UnauthorizedError("You are not authorized to update this watchparty");
    }
    ...
    const updatedWatchparty = await this.repository.update(id, {
      ...
      scheduledAt: body.scheduledAt ? body.scheduledAt.toISOString() : undefined,
