...
CREATE TABLE watchparties (
    ...
    current_timestamp INTEGER DEFAULT 0, -- Position en secondes
...
CREATE TABLE watchparty_participants (
    ...
    CONSTRAINT unique_participant UNIQUE (watchparty_id, user_id)
);
...
CREATE TRIGGER update_watchparties_updated_at BEFORE UPDATE ON watchparties
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
CREATE TRIGGER update_list_count AFTER INSERT OR DELETE ON list_items
...
INSERT INTO streaming_platforms (name, slug, base_url, is_supported) VALUES
    ('Netflix', 'netflix', 'https://www.netflix.com', true),
    ('Disney+', 'disney-plus', 'https://www.disneyplus.com', true),
    ('Prime Video', 'prime-video', 'https://www.primevideo.com', true),
    ('HBO Max', 'hbo-max', 'https://www.max.com', false),
    ('Apple TV+', 'apple-tv', 'https://tv.apple.com', false);
...
LEFT JOIN watchparty_participants wpp ON wp.id = wpp.watchparty_id AND wpp.status = 'confirmed'
WHERE wp.status = 'scheduled' AND wp.scheduled_at > NOW()
GROUP BY wp.id, u.username, c.title, sp.name;
