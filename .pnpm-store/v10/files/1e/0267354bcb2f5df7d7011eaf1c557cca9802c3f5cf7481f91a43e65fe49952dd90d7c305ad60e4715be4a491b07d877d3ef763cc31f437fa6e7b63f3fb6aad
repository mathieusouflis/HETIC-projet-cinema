...
const generateSwrHook = ({ queryParams, operationName, body, props, verb, params, override, mutator, response, summary, deprecated }, { route, context }) => {
	...
	if (verb === Verbs.GET) {
		const swrKeyProperties$1 = props.filter((prop) => prop.type !== GetterPropType.HEADER).map((param) => {
		...
		const queryKeyProps$1 = toObjectString(props.filter((prop) => prop.type !== GetterPropType.HEADER), "implementation");
...
//#endregion
export { builder, src_default as default, generateSwr, generateSwrHeader, getSwrDependencies };
